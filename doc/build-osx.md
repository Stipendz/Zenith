...
====================================
This guide will show you how to build SPDZd (headless client) for OSX.
...
4.  (Optional) You can also install SPDZd to your path:
...
4. Enter "SPDZ-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `SPDZd` for your own use.

SPDZd/SPDZ-cli binaries are not included in the SPDZ-Qt.app bundle.

If you are building `SPDZd` or `SPDZ-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the SPDZ-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./SPDZd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./SPDZd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=SPDZrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/SPDZ/SPDZ.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/SPDZ/SPDZ.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/SPDZ/debug.log

Other commands:
-------

    ./SPDZd -daemon # to start the SPDZ daemon.
    ./SPDZ-cli --help  # for a list of command-line options.
    ./SPDZ-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure SPDZ with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the SPDZ wallet:
...
You have the choice to build the GUI SPDZ wallet as a Mac OSX app, described in “How to build the SPDZ-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the SPDZ-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find SPDZ-Qt.dmg inside your SPDZ folder. Open and install the wallet like any typical Mac app.
