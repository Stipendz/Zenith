...
CzSPDZWallet::CzSPDZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zSPDZ seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzSPDZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezSPDZSeed_deprecated()) {
                    LogPrintf("%s: Updated zSPDZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zSPDZ seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zSPDZ wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSPDZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezSPDZCount(nCountLastUsed);
    else if (!walletdb.ReadzSPDZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSPDZWallet::Lock()
{
    seedMaster = 0;
}

void CzSPDZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSPDZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozSPDZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSPDZWallet::LoadMintPoolFromDB()
{
...
void CzSPDZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSPDZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSPDZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zSPDZTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSPDZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozSPDZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zSPDZTracker which also adds to database
    pwalletMain->zSPDZTracker->Add(dMint, true);
...
void CzSPDZWallet::SeedTozSPDZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSPDZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSPDZWallet::GenerateDeterministiczSPDZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSPDZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSPDZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
