...
#ifndef SPDZ_zSPDZWALLET_H
#define SPDZ_zSPDZWALLET_H
...
public:
    CzSPDZWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczSPDZ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozSPDZ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SPDZ_zSPDZWALLET_H
