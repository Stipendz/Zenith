...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"SPDZprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"SPDZprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SPDZ address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"SPDZaddress\"\n"
            "\nReveals the private key corresponding to 'SPDZaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"SPDZaddress\"   (string, required) The SPDZ address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by SPDZ %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"SPDZaddress\"\n"
            "\nEncrypts a private key corresponding to 'SPDZaddress'.\n"
            "\nArguments:\n"
            "1. \"SPDZaddress\"   (string, required) The SPDZ address for the private key (you must hold the key already)\n"
            "2. \"passSPDZase\"   (string, required) The passSPDZase you want the private key to be encrypted with - Valid special chars: !#$%&'()*+,-./:;<=>?`{|}~ \n"
            "\nResult:\n"
    ...
    string strAddress = params[0].get_str();
    string strPassSPDZase = params[1].get_str();
    ...
    string encryptedOut = BIP38_Encrypt(strAddress, strPassSPDZase, privKey, vchSecret.IsCompressed());
...
UniValue bip38decrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38decrypt \"SPDZaddress\"\n"
            "\nDecrypts and then imports password protected private key.\n"
            "\nArguments:\n"
            "1. \"encryptedkey\"   (string, required) The encrypted private key\n"
            "2. \"passSPDZase\"   (string, required) The passSPDZase you want the private key to be encrypted with\n"
    ...
    /** Collect private key and passSPDZase **/
    string strKey = params[0].get_str();
    string strPassSPDZase = params[1].get_str();

    uint256 privKey;
    bool fCompressed;
    if (!BIP38_Decrypt(strPassSPDZase, strKey, privKey, fCompressed))
