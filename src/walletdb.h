...
public:
    ...
    bool WritezSPDZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzSPDZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzSPDZSeed_deprecated(uint256& seed);
    bool ErasezSPDZSeed();
    bool ErasezSPDZSeed_deprecated();

    bool WritezSPDZCount(const uint32_t& nCount);
    bool ReadzSPDZCount(uint32_t& nCount);
