...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("SPDZ Core SPDZ-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  SPDZ-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded SPDZ transaction") + "\n" +
                               "  SPDZ-tx [options] -create [commands]   " + _("Create hex-encoded SPDZ transaction") + "\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded SPDZ transaction
