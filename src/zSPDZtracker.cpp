...
CzSPDZTracker::CzSPDZTracker(std::string strWalletFile)
{
...
CzSPDZTracker::~CzSPDZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSPDZTracker::Init()
{
...
bool CzSPDZTracker::Archive(CMintMeta& meta)
{
...
bool CzSPDZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSPDZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSPDZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSPDZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSPDZTracker::GetSerialHashes()
{
...
CAmount CzSPDZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSPDZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSPDZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSPDZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSPDZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSPDZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSPDZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSPDZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSPDZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSPDZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSPDZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPDZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPDZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSPDZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSPDZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSPDZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSPDZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzSPDZ from DB\n", __func__, listDeterministicDB.size());
    }
