                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSPDZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zSPDZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSPDZSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSPDZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSPDZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSPDZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passSPDZase first."));
            return;
        }
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSPDZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSPDZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzSPDZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSPDZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSPDZ
        sendzSPDZ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSPDZ
    sendzSPDZ();
}

void PrivacyDialog::on_pushButtonzSPDZControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    zSPDZControlDialog* zSPDZControl = new zSPDZControlDialog(this);
    zSPDZControl->setModel(walletModel);
    zSPDZControl->exec();
}

void PrivacyDialog::setzSPDZControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSPDZSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSPDZ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SPDZ Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSPDZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSPDZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SPDZ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSPDZ</b>";
    ...
    // use mints from zSPDZ selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!zSPDZControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zSPDZControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSPDZ require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zSPDZ"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == zSPDZ_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSPDZ require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zSPDZ"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSPDZ was spent successfully update the addressbook with the label
    ...
    // Clear zSPDZ selector in case it was used
    zSPDZControlDialog::setSelectedMints.clear();
    ui->labelzSPDZSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSPDZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SPDZ, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSPDZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zSPDZTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSPDZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSPDZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSPDZ "));
    ui->labelzSPDZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSPDZSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSPDZ->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSPDZ
        ui->pushButtonMintzSPDZ->setEnabled(false);
        ui->pushButtonMintzSPDZ->setToolTip(tr("zSPDZ is currently disabled due to maintenance."));

        // Spend zSPDZ
        ui->pushButtonSpendzSPDZ->setEnabled(false);
        ui->pushButtonSpendzSPDZ->setToolTip(tr("zSPDZ is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSPDZ
        ui->pushButtonMintzSPDZ->setEnabled(true);
        ui->pushButtonMintzSPDZ->setToolTip(tr("PrivacyDialog", "Enter an amount of SPDZ to convert to zSPDZ", 0));

        // Spend zSPDZ
        ui->pushButtonSpendzSPDZ->setEnabled(true);
        ui->pushButtonSpendzSPDZ->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}