...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SPDZ);
    ...
    if (!settings.contains("nAnonymizeSPDZAmount"))
        settings.setValue("nAnonymizeSPDZAmount", 1000);

    nAnonymizeSPDZAmount = settings.value("nAnonymizeSPDZAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSPDZAmount"))
        SoftSetArg("-anonymizeSPDZamount", settings.value("nAnonymizeSPDZAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in SPDZ.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSPDZAmount:
            return QVariant(nAnonymizeSPDZAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSPDZAmount:
            nAnonymizeSPDZAmount = value.toInt();
            settings.setValue("nAnonymizeSPDZAmount", nAnonymizeSPDZAmount);
            emit anonymizeSPDZAmountChanged(nAnonymizeSPDZAmount);
            break;
        case CoinControlFeatures:
