...
bool isValidPassSPDZase(QString strPassSPDZase, QString& strInvalid)
{
    for (int i = 0; i < strPassSPDZase.size(); i++) {
        if (!validChar.contains(strPassSPDZase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPassSPDZase[i]))
                continue;

            strInvalid = strPassSPDZase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPassSPDZase = ui->passSPDZaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassSPDZase(qstrPassSPDZase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passSPDZase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(EncodeDestination(addr), qstrPassSPDZase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPassSPDZase = ui->passSPDZaseIn_DEC->text().toStdString();
    ...
    if (!BIP38_Decrypt(strPassSPDZase, strKey, privKey, fCompressed)) {
        ...
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passSPDZase and try again."));
        return;
    }
