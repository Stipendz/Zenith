...
#include "ui_zSPDZcontroldialog.h"
...
std::set<std::string> zSPDZControlDialog::setSelectedMints;
std::set<CMintMeta> zSPDZControlDialog::setMints;

zSPDZControlDialog::zSPDZControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::zSPDZControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zSPDZControlDialog::~zSPDZControlDialog()
{
    delete ui;
}

void zSPDZControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zSPDZControlDialog::updateList()
{
...
void zSPDZControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zSPDZControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzSPDZ_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzSPDZControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zSPDZControlDialog::GetSelectedMints()
{
...
void zSPDZControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
