...
VERSION=
commit=false
url=https://github.com/SPDZproject/SPDZ
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the SPDZ, gitian-builder, gitian.sigs, and SPDZ-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/SPDZproject/SPDZ
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit SPDZ=${COMMIT} --url SPDZ=${url} ../SPDZ/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../SPDZ/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/SPDZ-*.tar.gz build/out/src/SPDZ-*.tar.gz ../SPDZ-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit SPDZ=${COMMIT} --url SPDZ=${url} ../SPDZ/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/SPDZ-*.zip build/out/SPDZ-*.exe ../SPDZ-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit SPDZ=${COMMIT} --url SPDZ=${url} ../SPDZ/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/SPDZ-*.tar.gz build/out/SPDZ-*.dmg ../SPDZ-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit SPDZ=${COMMIT} --url SPDZ=${url} ../SPDZ/contrib/gitian-descriptors/gitian-aarch64.yml
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../SPDZ/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/SPDZ-*win64-setup.exe ../SPDZ-binaries/${VERSION}
	    mv build/out/SPDZ-*win32-setup.exe ../SPDZ-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../SPDZ/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/SPDZ-osx-signed.dmg ../SPDZ-binaries/${VERSION}/SPDZ-${VERSION}-osx.dmg
	fi
	popd || exit
